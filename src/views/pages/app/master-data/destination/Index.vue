<template>
  <div>
    <a-button type="primary" shape="circle" icon="plus" style="margin-bottom: 2rem" @click="createDestination"/>
    <a-table 
        :columns="columns"
        :data-source="data"
    ...
    >
        <span slot="action" slot-scope="record">
            <a-button type="primary" shape="circle" icon="edit" ghost @click="updateDestination(record.id)"/>
            <a-divider type="vertical" />
            <a-popconfirm placement="topRight" title="Kamu yakin menghapus nya?" ok-text="Ya" cancel-text="Tidak" @confirm="deleteDestination(record.id)">
...
export default {
            ...
            columns: [
                {
                    ...
                    sorter: (value1, value2) => value1.destination_name.localeCompare(value2.destination_name),
    ...
    methods: {
        async getDestinationData(){
            try {
                const { data: {dataDestination} } = await this.$http({
                    method: "GET",
                    url: "/destination",
                });
